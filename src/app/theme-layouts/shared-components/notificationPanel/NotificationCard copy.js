import Card from '@mui/material/Card';
import Box from '@mui/material/Box';
import IconButton from '@mui/material/IconButton';
import Typography from '@mui/material/Typography';
import clsx from 'clsx';
import FuseSvgIcon from '@fuse/core/FuseSvgIcon';
import NavLinkAdapter from '@fuse/core/NavLinkAdapter';

function NotificationCard(props) {
  const { item, className } = props;
  const variant = item?.variant || '';

  const handleClose = (ev) => {
    ev.preventDefault();
    ev.stopPropagation();

    if (props.onClose) {
      props.onClose(item.id);
    }
  };

  return (
    <Card
      className={clsx(
        'flex items-center relative w-full rounded-16 p-12 min-h-72 shadow space-x-8',
        variant === 'success' && 'bg-green-600 text-white',
        variant === 'info' && 'bg-blue-700 text-white',
        variant === 'error' && 'bg-red-600 text-white',
        variant === 'warning' && 'bg-orange-600 text-white',
        className
      )}
      elevation={0}
      component={item.useRouter ? NavLinkAdapter : 'div'}
      to={item.link || ''}
      role={item.link && 'button'}
    >
      {item.icon && !item.image && (
        <Box
          sx={{ backgroundColor: 'background.default' }}
          className="flex shrink-0 items-center justify-center w-32 h-32 mr-12 rounded-full"
        >
          <FuseSvgIcon className="opacity-75" color="inherit">
            {item.icon}
          </FuseSvgIcon>
        </Box>
      )}

      {item.channel_image && (
        <img
          className="shrink-0 w-40 h-54 mr-12 rounded-lg overflow-hidden object-fill object-center"
          src={`assets/images/apps/tickers/logos/${item.channel_image}`}
          alt="Notification"
        />
      )}

      <div className="flex flex-col flex-auto">
        {item && (
          <div
            className="line-clamp-2 text-xl h-72 text-end pr-24 urdu-text"
            dangerouslySetInnerHTML={{ __html: item.text_ocr }}
          />
        )}

        {item && (
          <Typography className="mt-4 text-md leading-none " color="text.secondary">
            <span className="flex justify-end items-end mr-16">
              <span>
                <span className="mr-16">{item.date}</span>
                {item.time}
              </span>

              {/* {formatDistanceToNow(new Date(item.date), { addSuffix: true })} */}
            </span>
          </Typography>
        )}
      </div>

      <IconButton
        disableRipple
        className="top-0 right-0 absolute p-8"
        color="inherit"
        size="small"
        onClick={handleClose}
      >
        <FuseSvgIcon size={12} className="opacity-75" color="inherit">
          heroicons-solid:x
        </FuseSvgIcon>
      </IconButton>
      {item.children}
    </Card>
  );
}

export default NotificationCard;
